...
class ExportWorker(QThread):
    ...
    def _is_memory_item(self, filepath):
        """Check if a filepath is a memory item"""
        return filepath in self.memory_items
    ...
    def _copy_file(self, src_path, dest_path):
        """Copy a file, handling memory items by saving dataset to disk"""
        if src_path in self.memory_items:
            ds = self.memory_items[src_path]
            ds.save_as(dest_path, write_like_original=False)
        else:
            shutil.copy2(src_path, dest_path)

    def _write_to_zip(self, zipf, filepath, arcname):
        """Write a file to ZIP, handling memory items"""
        if filepath in self.memory_items:
            ds = self.memory_items[filepath]
            # Write dataset to bytes buffer
            buffer = io.BytesIO()
            ds.save_as(buffer, write_like_original=False)
            buffer.seek(0)
            zipf.writestr(arcname, buffer.read())
        else:
        ...
        try:
            with zipfile.ZipFile(self.output_path, 'w', compression=zipfile.ZIP_DEFLATED) as zipf:
                for idx, fp in enumerate(self.filepaths):
                    ...
                    try:
                        self._write_to_zip(zipf, fp, os.path.basename(fp))
                        zipped_count += 1
    ...
    def _export_dicomdir_zip(self):
        ...
        try:
            ...
            file_mapping = path_generator.generate_paths(self.filepaths, self.memory_items)
            ...
            builder = DicomdirBuilder("DICOM_EXPORT", self.memory_items)
            ...
            # Calculate statistics (handle memory items)
            total_size = 0
            for f in self.filepaths:
                if f in self.memory_items:
                    # Estimate size for memory items
                    try:
                        buffer = io.BytesIO()
                        self.memory_items[f].save_as(buffer, write_like_original=False)
                        total_size += buffer.tell()
                    except Exception:
                        pass  # Skip if we can't estimate
    ...
    def _copy_files_to_dicom_structure(self, file_mapping):
        ...
        for idx, (original_path, dicom_path) in enumerate(file_mapping.items()):
            ...
            try:
                ...
                # Copy file (handles memory items)
                self._copy_file(original_path, full_target_path)
    ...
    def _extract_patient_ids(self):
        ...
        for fp in self.filepaths:
            try:
                ds = self._read_dicom(fp, stop_before_pixels=True)
